...
def product_content(request, id):
    items = ProductPolicies.objects.filter(id=id)
...
class gallery(BaseView):
    def get(self, request):
        ...
        context = {'social_info': social_info, 'categories': categories, 'cat': cat, 'cat2': cat2}

        return render(request, 'gallery.html', context)


def gallery_content(request):
    return render(request, 'gallery_content.html')
...
def career_page(request):
    ...
    return render(request, 'career.html', {'categories': categories})
